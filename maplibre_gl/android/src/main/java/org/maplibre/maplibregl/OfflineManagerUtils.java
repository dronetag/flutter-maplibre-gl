  ...
  private static final String TAG = "OfflineManagerUtils";
    ...
    private static OfflineRegionDefinition mapToRegionDefinition(
        ...
        if (map.containsKey("geometry")) {
            String geometryJson = new Gson().toJson(map.get("geometry"));
            ...
            Geometry geometry = MultiPolygon.fromJson(geometryJson);
    ...
    private static Map<String, Object> offlineRegionDefinitionToMap(
        ...
        if (definition.getType().equals("tileregion")) {
            result.put("bounds", boundsToList(definition.getBounds()));
        } else if (definition.getType().equals("shaperegion") && definition instanceof OfflineGeometryRegionDefinition) {
            Geometry geometry = (((OfflineGeometryRegionDefinition) definition)).getGeometry();
            result.put("geometry", new Gson().fromJson(geometry.toJson(), HashMap.class));
        }
